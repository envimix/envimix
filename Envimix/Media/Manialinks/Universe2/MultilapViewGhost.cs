...
public class MultilapViewGhost : CTmMlScriptIngame, IContext
{
    public int Start;
    ...
    [Netread] public required int CurrentNbLaps { get; set; }

    bool IsVisible()
    {
        return MapIsLapRace && MapNbLaps > 1;
    }
    ...
    public void Loop()
    {
        ...
        if (CurrentNbLaps >= NbLaps)
        {
            LabelMultilap.Value = $"🔁  {CurrentNbLaps} / {NbLaps}";
        }
        else
        {
            LabelMultilap.Value = $"🔁  {CurrentNbLaps + 1} / {NbLaps}";
        }
    }
}
